...
export const SupportMessageSchema = z.object({
	...
	problemDescription: z
		.string()
		.min(1, { message: 'Поле не может быть пустым' })
		.max(450, { message: 'Максимальная длина 450 символов' })
		.optional(),
	handleChange: z.function().optional(),
});

export async function SupportMessageFormOnSubmit(
...
export async function SupportMessageAnswerFormOnSubmit(
	values: z.infer<typeof SupportMessageAnswerSchema>
) {
	const fetchMessage = async () => {
	...
	return fetchMessage()
		.then(() => toast.success('Ответ отправлен!'))
		.catch(() => toast.error('Что-то пошло не так.'));
}
